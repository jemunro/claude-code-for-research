...
\small \textbf{MCP} (Model Context Protocol) — a shared interface that lets Claude call out to external services and data sources. \normalsize
...
## \texttt{fraction = 0.5} should keep ranges with at least 50\% overlap
...
\colorlet{shadecolor}{wehiPink!18}
...
\colorlet{shadecolor}{wehiCodeBg}
:::

::: {.column width="49%"}
Route through \texttt{overlap\_hits} when a fraction is set.

\colorlet{shadecolor}{wehiGreen!18}
...
Run \textbf{open-weight models} (e.g.\ the \textbf{Qwen} family) on our HPC for work involving controlled-access data.
\end{block}
...
If you were on the list, you should've received an invite to join Bahlo lab team: 
\begin{center}
\includegraphics[width=0.72\textwidth, height=0.62\textheight, keepaspectratio]{figures/email-invite.png}
\end{center}
